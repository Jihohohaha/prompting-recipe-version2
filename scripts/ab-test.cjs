// scripts/ab-test.js
// Simple Playwright script to visit four AB variants and capture console logs.
// Usage:
//   npm i -D playwright
...
(async () => {
  ...
  // Only test the primary ratio value (0.4) to reduce noise per request
  const ratioVariants = [0.4];
  const baseVariants = [
    { name: 'baseline', qs: '' },
  ...
  // expand variants to include ratio query param
  const variants = [];
  ...
  for (const v of variants) {
    ...
    try {
      ...
      while (Date.now() - start < timeoutMs) {
        if (logs.find(l => l.includes('centering COMPLETE') || l.includes('Scroll completed to'))) {
          seenComplete = true;
          break;
        }
      ...
      await new Promise(r => setTimeout(r, seenComplete ? 1200 : 2000));
